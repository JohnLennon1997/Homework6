...
public class DecisionTree {
	...
	public DecisionTree(File file) throws FileNotFoundException{
		...
		this.root = DecisionTreeHelper(inputScanner);
	}
	
	public DecisionNode DecisionTreeHelper(Scanner in) {
		String input = in.next();	
		if (input.contains("#")) {
			DecisionNode tobeAdded = new QuestionNode(input.substring(1, input.length()), DecisionTreeHelper(in), DecisionTreeHelper(in));
			return tobeAdded;
		}
		else{
			DecisionNode tobeAdded = new GuessNode(input);
			return tobeAdded;
		}
	}
